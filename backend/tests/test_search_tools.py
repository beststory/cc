...
class TestCourseSearchTool:
    ...
    def test_execute_successful_search(
        self, mock_vector_store_with_data, sample_search_results
    ):
        ...
        mock_vector_store_with_data.search.assert_called_once_with(
            query="MCP", course_name="MCP 서버 개발 가이드", lesson_number=None
        )
    ...
    def test_execute_with_lesson_filter(self, mock_vector_store_with_data):
        ...
        mock_vector_store_with_data.search.assert_called_once_with(
            query="설정", course_name=None, lesson_number=2
        )
...
class TestCourseSyllabulTool:
    ...
    def test_execute_successful_syllabus_retrieval(
        self, mock_vector_store_with_data, sample_course
    ):
