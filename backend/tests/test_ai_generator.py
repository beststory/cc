...
class TestAIGenerator:
    ...
    @patch('anthropic.Anthropic')
    def test_generate_response_with_tools_no_tool_use(self, mock_anthropic_class, mock_anthropic_client, mock_tool_manager):
    ...
    @patch('anthropic.Anthropic')
    def test_generate_response_with_tool_use(self, mock_anthropic_class, mock_anthropic_client_with_tool_use, mock_tool_manager):
    ...
    def test_generate_response_with_conversation_history(self, mock_anthropic_class, mock_anthropic_client):
        ...
        response = generator.generate_response("MCP란 무엇인가요?", conversation_history=history)
    ...
    def test_handle_tool_execution_multiple_tools(self, mock_tool_manager):
        ...
        mock_tool_manager.execute_tool.assert_any_call("get_course_syllabus", course_name="test2")
    ...
    @patch('anthropic.Anthropic')
    def test_api_parameters_structure(self, mock_anthropic_class, mock_anthropic_client):
    ...
    @patch('anthropic.Anthropic')
    def test_sequential_tool_calls_two_rounds_success(self, mock_anthropic_class, mock_tool_manager):
        ...
        final_response.content = [Mock(text="MCP는 Model Context Protocol로 AI 모델과 외부 도구 간의 표준화된 통신 프로토콜입니다.")]
        ...
        # 검증: 2번의 도구 실행
        assert mock_tool_manager.execute_tool.call_count == 2
        mock_tool_manager.execute_tool.assert_any_call("search_course_content", query="MCP 기본개념", course_name="MCP 서버 개발")
        mock_tool_manager.execute_tool.assert_any_call("get_course_syllabus", course_name="MCP 서버 개발")
        ...
        assert result == "MCP는 Model Context Protocol로 AI 모델과 외부 도구 간의 표준화된 통신 프로토콜입니다."
    
    @patch('anthropic.Anthropic')
    def test_tool_call_then_normal_completion(self, mock_anthropic_class, mock_tool_manager):
    ...
    @patch('anthropic.Anthropic')
    def test_max_tool_rounds_limit_reached(self, mock_anthropic_class, mock_tool_manager):
        """최대 2라운드 제한 도달 테스트"""
        mock_client = Mock()
        mock_anthropic_class.return_value = mock_client
    ...
    @patch('anthropic.Anthropic') 
    def test_tool_failure_in_middle_round(self, mock_anthropic_class, mock_tool_manager):
    ...
    @patch('anthropic.Anthropic')
    def test_api_call_count_and_message_structure_validation(self, mock_anthropic_class, mock_tool_manager):
