...
class TestRAGSystem:
    ...
    def mock_components(self):
        ...
        components['document_processor'].process_course_document.return_value = (Mock(), [Mock()])
    ...
    def test_rag_system_initialization(self, mock_syllabus_tool, mock_search_tool, mock_tool_manager,
                                      mock_doc_processor, mock_vector_store, mock_ai_generator,
                                      mock_session_manager, test_config):
        ...
        mock_ai_generator.assert_called_once_with(test_config.ANTHROPIC_API_KEY, test_config.ANTHROPIC_MODEL)
        ...
        assert mock_tool_manager.return_value.register_tool.call_count == 2  # search_tool and syllabus_tool
    ...
    def test_query_content_question_workflow(self, mock_components, test_config):
        ...
        with patch.multiple('rag_system',
            ...
            mock_components['tool_manager'].get_last_sources.return_value = [
                {"text": "MCP 서버 개발 가이드 - Lesson 1", "link": "https://example.com/lesson1"}
            ]
            ...
            assert call_args[1]['tools'] == mock_components['tool_manager'].get_tool_definitions()
    ...
    def test_query_with_session_management(self, mock_components, test_config):
        ...
        with patch.multiple('rag_system',
            ...
            mock_components['session_manager'].get_conversation_history.return_value = "Previous: Hello"
            
            # Execute query with session ID
            response, sources = rag_system.query("후속 질문입니다", session_id="test_session")
            
            # Verify session management calls
            mock_components['session_manager'].get_conversation_history.assert_called_once_with("test_session")
            mock_components['session_manager'].add_exchange.assert_called_once_with("test_session", "후속 질문입니다", "Test AI response")
    ...
    def test_add_course_folder_success(self, mock_isfile, mock_join, mock_listdir, mock_exists,
                                      mock_components, test_config, sample_course, sample_course_chunks):
        ...
        mock_components['document_processor'].process_course_document.return_value = (sample_course, sample_course_chunks)
    ...
    def test_add_course_folder_duplicate_prevention(self, mock_components, test_config, sample_course):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            mock_components['vector_store'].get_existing_course_titles.return_value = {sample_course.title}
            mock_components['document_processor'].process_course_document.return_value = (sample_course, [])
    ...
    def test_add_single_course_document_success(self, mock_components, test_config, sample_course, sample_course_chunks):
        ...
        with patch.multiple('rag_system',
            ...
            mock_components['document_processor'].process_course_document.assert_called_once_with("/test/course.txt")
            mock_components['vector_store'].add_course_metadata.assert_called_once_with(sample_course)
            mock_components['vector_store'].add_course_content.assert_called_once_with(sample_course_chunks)
    ...
    def test_add_single_course_document_error(self, mock_components, test_config):
        ...
        mock_components['document_processor'].process_course_document.side_effect = Exception("Processing error")
    ...
    def test_get_course_analytics(self, mock_components, test_config):
        ...
        mock_components['vector_store'].get_existing_course_titles.return_value = ["Course 1", "Course 2", "Course 3"]
